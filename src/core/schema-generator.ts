...
/**
 * Generates DDL for the _token_transfers table.
 * Stores decoded SPL Token transfer instructions (Transfer, TransferChecked,
 * MintTo, Burn) extracted from indexed transactions.
 */
export function generateTokenTransfersTable(): string {
  return `
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_token_transfers_tx ON _token_transfers(tx_signature, ix_index, COALESCE(inner_ix_index, -1));`.trim();
}
...
export function generateDDL(parsed: ParsedIDL, config: ProgramConfig): string[] {
  ...
  // Include token transfers table if enabled
  if (config.tokenTransfers) {
    ddl.push(generateTokenTransfersTable());
  }
