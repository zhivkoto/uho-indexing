...
// =============================================================================
// Codama IDL Types (generated by Codama / Kinobi toolchain)
// =============================================================================

/** Top-level Codama IDL structure */
export interface CodamaIDL {
  version: string;
  name: string;
  address?: string;
  instructions: CodamaInstruction[];
  accounts?: CodamaAccountDef[];
  types?: CodamaTypeDef[];
  metadata?: {
    origin?: string;
    address?: string;
...
/** Codama instruction argument */
export interface CodamaArg {
  name: string;
  type: string | { defined: string } | { vec: string } | { option: string };
}

/** Codama account definition */
export interface CodamaAccountDef {
  name: string;
  discriminator?: number[];
}

/** Codama type definition */
export interface CodamaTypeDef {
  name: string;
  type: {
    kind: string;
...
export interface ProgramConfig {
  ...
  idl: string;
  events?: string[];
  /** Enable cross-cutting token transfer tracking for this program's transactions */
  tokenTransfers?: boolean;
}

/** A decoded token transfer extracted from a Solana transaction */
export interface DecodedTokenTransfer {
  programId: string;         // Token program ID (SPL Token or Token-2022)
  instructionType: string;   // 'transfer' | 'transferChecked' | 'mintTo' | 'burn'
  source: string;            // Source token account pubkey
  destination: string;       // Destination token account pubkey
  authority: string;         // Signer authority pubkey
  mint: string | null;       // Mint pubkey (for Checked variants)
  amount: string;            // Amount as string (u64)
  decimals: number | null;   // Decimals (for Checked variants)
  slot: number;
  blockTime: number | null;
  txSignature: string;
  ixIndex: number;
